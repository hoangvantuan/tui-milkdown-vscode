...
- ~~Screen reader feedback cho trạng thái "copied"~~ — resolved: `copySvgAsPng` helper swaps `aria-label` to "Copied" for 1.5s.
- ~~Refactor duplicate copy flow~~ — resolved: shared `copySvgAsPng()` helper in `svg-to-png.ts`.
...
4. ~~**`copyFonts()` crash nếu node_modules chưa cài**~~ — resolved: function removed in Stage 4 export rewrite (puppeteer-core, no bundled fonts).
...
4. ~~**Debounce timer not cancelled on widget destroy**~~ — resolved (2026-04-22): timers cleared in `apply()` when `tr.docChanged`, before `buildDecorations` recreates widgets.
5. ~~**Duplicate SVG element IDs across cached previews**~~ — resolved (2026-04-22): `uniquifySvgIds()` rewrites all IDs with per-widget prefix after every render/cache hit.
