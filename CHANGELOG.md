...
- **Cursor and scroll position are remembered per file (#121)**. #84 said to use `setState`, and that is the wrong store: there is no `retainContextWhenHidden` on this webview, so hiding the tab destroys it and takes `vscode.setState` with it. The position lives in the extension host's `workspaceState`, keyed by document URI, and is replayed on `ready` exactly the way the saved theme, font and zoom already are.
...
- **Reading time next to the word count, and a copy-anchor button on hover over a heading** (the last two bullets of #84). The GitHub-style slug rule was inline inside `scrollToHeading`; it is now `headingSlug()` in `heading-level-plugin.ts`, next to the button that emits those strings, because two copies of that rule would drift and the symptom would be a copied anchor that silently scrolls nowhere.
...
- **The floor check drives every 2.17 surface and grew from 20 checks to 29.** The wave closed ten issues whose behaviour exists only against a live ProseMirror view, and it closed them on fourteen hand-test criteria, which is a debt rather than evidence. `driveSurfaces` in `harness/vscode-floor/run.mjs` now types `/` and reads the slash menu, opens find-and-replace and replaces a word, selects text and reads the bubble menu, drags an image handle and reads back the written width, opens and Escapes the lightbox, hovers a heading for its anchor button, and reads the reading time; `extension-tests.ts` adds the editor-association check. Teeth were measured in two batches, exactly 3 and exactly 4 red against the corresponding code removed. Three probes accused working code before they were correct, and those corrections are the useful part: an image NodeView writes `style.width` rather than a `width` attribute, a hover overlay needs a `mousemove` carrying coordinates inside the target rect, and `defaultPrevented` on a `contextmenu` proves nothing because the VS Code webview cancels that event itself. Two criteria stay hand-only and are named rather than faked: Git Graph's diff view (#48, a different path in VS Code, and the extension is not installed in the floor workspace) and the appearance of the menus.
