...
- **Code blocks redrawn**: the accent bar at the top is a solid 2px line instead of a gradient fading to transparent, the block has a faint 1px border, the header is a neutral tint rather than an accent one, the language name gets an accent dot, and focus shows a soft 3px ring instead of a hard 1px outline. The block is also leaner: a 32px header instead of 41px, 12px by 16px padding instead of 16px by 20px, and a 10px gap under the header instead of 12px. Those sizes are variables on `pre` that the header and the line-number gutter both read, so the table cell variant sets two variables instead of a padding the header could not see.
...
- **Line numbers drifted away from the code**: the gutter's line height was `1.5` of its own smaller font, 21.6px against the code's 24px lines, so the numbers slid up by a line roughly every eight lines. It now uses the code's line box. The header was also indented over the gutter when numbers were on, since it only reached back over the padding; it now reaches back over both. The gutter's `top` was a hard-coded 48px and is now derived from the header height.
...
- The insert opens the new block by setting `open` **on the DOM**, not in the document. A fresh node is closed, the UA hides every child but the `<summary>`, and the caret would land where nobody can see it; setting `attrs.open` instead would put ` open` into the file, and the NodeView deliberately never dispatches on toggle, so nothing could take it back out. Opening it as a DOM mutation is the same path a reader's click takes, and `ignoreMutation` already swallows it. The slash seam's `[details]` line records `<details>` with no ` open`, which is what pins it.

### Fixed

- **Clicking the code block's line-wrap button froze the editor until the tab was closed**: the handler wrote `code-wrap` onto the `<pre>`, DOM ProseMirror owns. The observer saw it, flushed, re-rendered, rebuilt the header widget, and the widget's restore step wrote the class again from a `queueMicrotask`, which runs after the observer is re-enabled. Measured in a live webview: `attributes on PRE attr=class` on every round, `classList.add(code-wrap)` from the restore step 60 times in 3 seconds, and `onTokenListValue=""` on every write, because each render hands out a fresh `<pre>`. A `contains()` guard therefore could not help and did not. The classes are a `Decoration.node` now, built from the same view state, so ProseMirror renders them itself and nothing writes from outside. Both `queueMicrotask` calls are gone and the toggles ask for a redraw with `tr.setMeta`, a transaction that changes no content and posts no `edit`.
- **The code block header's copy, language picker and line numbers all did nothing**: the header widget sits at `pos + 1`, so ProseMirror puts it INSIDE the `<code>` element, not the `<pre>`. Measured: `headerParent="CODE.language-js"`, `parentElement.querySelector("pre")` is `null`, `nextElementSibling` is a lowlight `<span>`. `resolveCodeBlockPos` fell through to that span and resolved a position inside the highlighted text, so copy came back empty and the language picker changed nothing; `updateLineNumbersGutter` called `pre.insertBefore(gutter, header.nextSibling)` with a node that is not a child of the `<pre>` and threw `NotFoundError`, killing the handler before it stored its state. Resolution is `header.closest("pre")` now, and the gutter is built inside the widget rather than inserted afterwards.
- These two hid each other. The line-numbers button did not freeze only because its handler threw before it could persist `lineNumbers`, so the restore step never ran; fixing the first defect alone makes that button freeze too, which is how the pair was confirmed. The floor check `a code block header button click leaves the renderer responsive` presses BOTH buttons with real mouse events and reads liveness after each, so the shielding cannot come back quietly. A JS `.click()` does not reproduce any of it.

- **An empty `<summary>` blew the stack on save**: `<details>\n<summary></summary>` in any file, hand-written or not, crashed serialization with `RangeError: Maximum call stack size exceeded` in 3.0.2. A node with no children has no `content` key at all, so `renderChildren(node)` falls through its array check and renders the node it was handed, which renders its own children again. `harness/fixtures/synthetic/details-empty-summary.md` is red before the fix. The guard is the one already used at `extension-factory.ts:373`.
