...
- **The floor check covers the new artifacts**: 36 checks to 49, and the six criteria these issues closed with as "verify by hand" are checked by the harness instead. KaTeX waits on the same budget as mermaid and the wait ends only when both are ready, so a fast mermaid cannot report a false red for a formula still loading.
