...
- **Mermaid lazy-load treo vĩnh viễn (#75)**: khi artifact nạp xong nhưng không đăng ký `window.__tuiMermaidBundle` (build hỏng hoặc cắt cụt), lần retry sau đó gắn listener `load` mới vào phần tử `<script>` đã fire `load` rồi (script chỉ fire `load`/`error` đúng một lần) nên promise không bao giờ settle, mọi diagram kẹt ở "Rendering…" không có lỗi. Giờ phần tử đã settle bị xóa khỏi DOM và lỗi "executed but did not register" được latch cho hết đời trang: các lần gọi sau reject ngay thay vì re-download bản hỏng (webview reload mới nhận build đã sửa); lỗi network vẫn xóa phần tử và giữ khả năng retry thật; mọi lỗi tải hiển thị qua `.mermaid-err-msg` sẵn có trong placeholder thay vì treo vô hạn. Caller đồng thời vẫn chia sẻ đúng một lần nạp và một lần đăng ký (chứng minh bằng probe jsdom 5 kịch bản)
