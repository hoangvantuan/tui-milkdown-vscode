...
- **Drag handle for block reordering (#133)**: hover a block, drag it. `@tiptap/extension-drag-handle` costs 127,420 B eagerly, most of it yjs and y-prosemirror reached through `@tiptap/extension-collaboration`, which this editor does not use, so it loads on the first hover instead. Upstream does not attach the handle to `#editor-container`; the plugin moves it there, because CSS `zoom` on `.tiptap` is transparent to the JS coordinate APIs.
...
- **Backlinks panel (#134)**: every workspace document that references the current one through a `[[wiki link]]` or an `@` mention, one click away.
...
- **The floor check covers the new artifacts**: 36 checks to 40. KaTeX waits on the same budget as mermaid and the wait ends only when both are ready, so a fast mermaid cannot report a false red for a formula still loading.
