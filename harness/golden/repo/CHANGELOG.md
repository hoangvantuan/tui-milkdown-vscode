...
- **A link wrapping an image survives a save (#98)**: `[![alt](src)](url)` was written back as `![alt](src)`. `@tiptap/extension-image` declares no `marks` on its node spec, so ProseMirror refused the `link` mark on an image and the parser dropped it silently. `MarkdownImage` now allows the mark, `MarkdownLink.parseMarkdown` attaches it to image leaf nodes as well as text nodes, and `MarkdownImage.renderMarkdown` writes the enclosing link back with the existing destination escaping. This repository's own README badge rows were losing their links on every save; the `repo/README.md` golden regains them. Pinned by `harness/fixtures/synthetic/image-in-link.md` (badge row, linked image with a title, destination with spaces). The upstream defect is written up but not yet filed.
