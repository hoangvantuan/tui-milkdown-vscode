...
- Insert: `insertFileMention()` replaces the `@query` range with an **inline** node — a text node carrying a `link` mark, the same shape wiki link uses. Escaping and the `<...>` wrapping for paths with spaces are left to `MarkdownLink.renderMarkdown` on save.
- Why not a markdown string: the insert used to be `insertContent("[name](<path>)", { contentType: "markdown" })`. That parses to a **paragraph**, and Tiptap's `insertContentAt` widens the replaced range for a block node only when the parent textblock is empty (`isEmptyTextBlock` in `@tiptap/core`). Inserted into a paragraph that already had text, the block split it and the link dropped onto its own line — the visible bug was "pick a file, press Enter, the link goes to a new line". The string path also escaped only `]`, so a filename containing `*`, `_` or `[` produced a broken link. Pinned by `harness/filemention-seam.ts` (golden `harness/golden/seams/file-mention.txt`), whose `mention-alone-empty-parent` case covers the empty-parent path that was always correct.
