...
- `harness/editor.ts` mirrors the markdown-relevant extensions of `initEditor()`; a change to one without the other makes the harness measure something that does not ship. It is NOT a full mirror, and the two can disagree: `harness/vscode-floor/sample.md` is not a first-pass fixed point under `roundtripMarkdown` and the webview USUALLY writes no edit for it, which is why the floor check usually passes. Usually, not always — the same SHA has produced both 18/19 and 19/19, and that nondeterminism is #111
- `verify:vscode-floor` is safe to run concurrently, since #110 — safe from corruption, which is not the same as reliable. Two runs at once are fine; three on one machine all fail `lazy mermaid artifact loads and renders` because the 40 s probe window is too short for three Electron instances rendering the lazy mermaid bundle (#112). Load was never the cause of the `document still unmodified after the hold` failure, and it is the whole cause of this one. It used to derive `ws`, `ud` and `ext` from one fixed base (`/tmp/tuimd-floor`), so two runs overwrote each other's `sample.md`, shared VS Code's user-data dir and IPC socket, and the second run's startup `rmSync` deleted the first run's tree. That, not machine load, is why `document still unmodified after the hold` failed spuriously during parallel agent waves. Each run now gets its own `mkdtemp` base and stages its writes into the shared download cache before renaming them into place
