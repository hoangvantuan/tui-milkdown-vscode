 ...
 * It exists so that W2 can fill in cases WITHOUT editing harness/roundtrip.ts,
 ...
 * table below is W2's to own, and nobody else's.
 *
 ...
 * extension under test changes a countable number of lines. See #S4.
 */
export function runLinkEditSeam(): string {
  const lines: string[] = [];
  lines.push("# link-edit-seam.ts — no cases yet");
  lines.push("");
  lines.push("This seam is a skeleton. W2 replaces this body with real cases.");
  return lines.join("\n") + "\n";
}
