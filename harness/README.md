...
| `math-footnote-seam.ts` | `seams/math-footnote.txt` | Math and footnote nodes: the LaTeX each holds as an ATTRIBUTE, and the markdown it serializes back to. An attribute is not text, which is why the escaper never reaches inside `$...$` (#131) |
| `html-render-seam.ts` | `seams/html-render.txt` | `<details>`/`<summary>` as nodes and `<kbd>`/`<sub>`/`<sup>` as MARKS, plus what each serializes to. Marks are what let a link wrap them, which an atom can never allow (#132) |
| `emoji-insert-seam.ts` | `seams/emoji-insert.txt` | What the picker writes for a `:shortcode:` and that a unicode character already in the file is never rewritten back into one. Verdicts, not measurements (#133) |
| `codeblock-seam.ts` | `seams/codeblock.txt` | Line numbers and wrap are view state, so the fenced block on disk is unchanged by either (#134) |
| `content-sync-seam.ts` | `seams/content-sync.txt` | Which edits the view posts to the host and which it does not: the #111 pair (a keystroke undone inside the debounce window, a document ending in an alert at load), baseline re-anchoring, debounce coalescing, and the extension-update latch read at three moments (#148, #150) |
| `image-path-seam.ts` | `seams/image-path.txt` | What the FILE gets for an image the view displays: a host map change, a rename driven by the plugin, and two nodes pointing at one image. The stale reverse cache that wrote a `vscode-resource` URL into a user's file is case three (#142, #147) |
...
The wave-7 seams (`list-keys` through `img-width`) were committed as SKELETONS,
with their registration lines in `roundtrip.ts`, before the worktrees were cut;
`content-sync` and `image-path` were cut the same way for wave 10. That is the pattern to
