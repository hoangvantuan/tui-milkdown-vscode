...
- Until #147 lands, `harness/image-path-seam.ts` imports `src/webview/main.ts` directly, which is why `main.ts` guards `acquireVsCodeApi()` and `init()` behind `typeof` checks and exports `setCurrentImageMap` / `_testResetReverseCache` (#142). Add no top-level side effect to `main.ts` that needs the DOM while that import exists; #147 moves the translation into its own module and removes the guards
